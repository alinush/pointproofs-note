...
Computing $\vect{y} = \matr{A}_N \cdot \vect{m}$ for any vector $m = [m_1,\dots, m_N]^\top$ can be done in $O(N\log{N})$ time, if $\matr{A}_N$ is a Toeplitz matrix.
The idea is to embed $\matr{A}_N$ inside a slightly bigger circulant matrix (see \cref{s:circulant}) in such a way that multiplying this circulant matrix by (an extension of) $\vect{m}$ can be used to compute $\matr{A}_N \cdot \vect{m}$.
